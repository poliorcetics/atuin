...
use super::record::{
    LoadingError, PACKFILE_TAG, PACKFILE_VERSION, PackManifest, PackManifestDataV1, StoringError,
};
...
pub async fn try_pack(
    ...
    let start = match &last_pack {
        Some(record) => match PackManifest::try_from(record)? {
            PackManifest::V1(v1) => v1.end_idx + 1,
        },
        None => 0,
    };
...
mod tests {
    ...
    async fn manifest_ranges(store: &SqliteStore, host: HostId) -> Vec<(u64, u64)> {
        store
            ...
            .unwrap()
            .iter()
            .map(|record| match PackManifest::try_from(record).unwrap() {
                PackManifest::V1(v1) => (v1.start_idx, v1.end_idx),
            })
            .collect()
    }
