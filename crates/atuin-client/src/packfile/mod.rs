...
//! packing operation, which will create a new [`record::PackManifest`] record in the table.
//!
//! ```txt
//! | idx | tag      | data (encrypted)         |
//! |   1 | history  | ...                      |
...
pub use download::{DownloadError, download_packed, download_packed_many};
pub use packer::{PackingError, try_pack};
pub use record::PACKFILE_TAG;
pub use upload::{UploadError, upload_packed, upload_packed_many};

#[cfg(test)]
pub(crate) use codec::{PackManifestRef, pack};
